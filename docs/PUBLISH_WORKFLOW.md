...
- 实际运行目录：`/path/to/runtime-worktree`
- 发布仓库目录：`/path/to/public-repo`
- GitHub 仓库：`git@github.com:<your-account>/<your-repo>.git`
...
```bash
cp /path/to/runtime-worktree/app.py /path/to/public-repo/app.py
cp -a /path/to/runtime-worktree/templates/. /path/to/public-repo/templates/
cp -a /path/to/runtime-worktree/static/. /path/to/public-repo/static/
```
...
diff -qr \
  ...
  --exclude data \
  --exclude docker-data \
  /path/to/runtime-worktree /path/to/public-repo
```
...
bash -n scripts/bootstrap-owner.sh
...
```bash
cd /path/to/public-repo
rg -n 'example\\.com|panel\\.example\\.com|BEGIN (RSA|EC|OPENSSH|PRIVATE) KEY|XRAY_REALITY_PRIVATE_KEY=.+[^$]' . \
  --glob '!data/**' \
  --glob '!docker-data/**' \
  --glob '!.venv/**' \
  --glob '!static/vendor/**'
...
- `scripts/bootstrap-owner.sh` 语法检查通过
